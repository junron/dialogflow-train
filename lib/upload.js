        ...
        if(existingIntents.includes(intentName)){
          if(responses[0]!==undefined){
            ...
            for(let i=0;i<newResponses.length;i++){
              const newRes = newResponses[i] === undefined ? "" : simplifyResponse(newResponses[i]).text.text[0]
              const oldRes = intent.messages[i] === undefined ? "" : simplifyResponse(intent.messages[i]).text.text[0]
              if(oldRes!==newRes){
        ...
        if(existingIntents.includes(intentName)){
          if(phrases[0]!==undefined){
            ...
            for(let i=0;i<Math.max(phrases.length,oldPhrases.length);i++){
              ...
              changed.push(JSON.stringify(difference))
          ...
          if(dryRun){
            output.push({
              type:"created",
              changes:[JSON.stringify(phrases.map(r=>{
                return {value:r,added:true}
              }))],
              intent:intentName
            })
